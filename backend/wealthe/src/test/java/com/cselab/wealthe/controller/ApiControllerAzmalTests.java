...
//----------------------------------------------------------------------
    ...
    @Test
    void testCalculateTaxEstimation_SuccessfulCalculation() {
        // Given - Properly setup SecurityContext
        Authentication mockAuth = mock(Authentication.class);
        when(mockAuth.getName()).thenReturn("123");
        ...
        requestData.put("expectedNonRecurringIncome", "50000.0");
        requestData.put("bonusAmount", "10000.0");
        requestData.put("numberOfBonus", "2");
        requestData.put("makeMoreInvestment", "25000.0");

        // Mock all database calls
        when(jdbcTemplate.queryForObject(contains("SELECT COALESCE(SUM(amount), 0) FROM income"),
                eq(Double.class), eq(123), any()))
                .thenReturn(30000.0);

        // ADD THIS - Mock date of birth for isElderly check
        when(jdbcTemplate.queryForObject(contains("SELECT date_of_birth FROM user_tax_info"),
                eq(String.class), eq(123)))
                .thenReturn("1990-01-01");

        List<Map<String, Object>> recurringIncomes = new ArrayList<>();
        Map<String, Object> monthlyIncome = new HashMap<>();
        monthlyIncome.put("amount", "15000.0");
        monthlyIncome.put("recurrence", "monthly");
        recurringIncomes.add(monthlyIncome);

        when(jdbcTemplate.queryForList(contains("SELECT amount, recurrence FROM income"), eq(123), any()))
                .thenReturn(recurringIncomes);

        List<Map<String, Object>> userTaxInfo = new ArrayList<>();
        Map<String, Object> taxInfo = new HashMap<>();
        taxInfo.put("is_ff", "false");
        taxInfo.put("is_female", "true");
        taxInfo.put("is_disabled", "false");
        userTaxInfo.add(taxInfo);

        when(jdbcTemplate.queryForList(contains("select is_ff, is_female, is_disabled"), eq(123)))
                .thenReturn(userTaxInfo);

        when(jdbcTemplate.queryForObject(contains("SELECT area_name FROM user_tax_info"), eq(String.class), eq(123)))
                .thenReturn("Dhaka");

        when(jdbcTemplate.queryForObject(contains("SELECT min_amount FROM rule_tax_zone_min_tax"), eq(Double.class), eq("Dhaka")))
                .thenReturn(5000.0);
        ...
        assertNotNull(result.getBody());
        assertTrue((Boolean) result.getBody().get("success"));
        assertEquals("Tax estimation calculated successfully", result.getBody().get("message"));
    }

    @Test
    void testCalculateTaxEstimation_SuccessfulCalculation_Simple() {
        // Given - Properly setup SecurityContext
        Authentication mockAuth = mock(Authentication.class);
        when(mockAuth.getName()).thenReturn("123");
        ...
        // Mock all database calls with more flexible matchers
        when(jdbcTemplate.queryForObject(contains("SELECT COALESCE(SUM(amount), 0) FROM income"),
                eq(Double.class), eq(123), any()))
                .thenReturn(30000.0);
        ...
        if (result.getStatusCode() == HttpStatus.INTERNAL_SERVER_ERROR) {
            ...
            if (errorBody != null) {
                System.out.println("Error message: " + errorBody.get("message"));
            }
    ...
    @Test
    void testCalculateTaxEstimation_UnauthorizedUser() {
        // Given
        when(mockAuth.getName()).thenReturn("0");
        ...
        assertNotNull(result.getBody());
        assertFalse((Boolean) result.getBody().get("success"));
        assertEquals("Unauthorized user", result.getBody().get("message"));

        verify(jdbcTemplate, never()).queryForObject(anyString(), (Object[]) any(), (int[]) any(), (RowMapper<Object>) any());
    }

    @Test
    void testCalculateTaxEstimation_InvalidExpectedNonRecurringIncome() {
        // Given
        when(mockAuth.getName()).thenReturn("123");

        Map<String, Object> requestData = new HashMap<>();
        requestData.put("expectedNonRecurringIncome", "invalid_number");
        ...
        assertNotNull(result.getBody());
        assertFalse((Boolean) result.getBody().get("success"));
        assertEquals("Invalid expected non-recurring income format", result.getBody().get("message"));
    }

    @Test
    void testCalculateTaxEstimation_InvalidBonusAmount() {
        // Given
        when(mockAuth.getName()).thenReturn("123");

        Map<String, Object> requestData = new HashMap<>();
        requestData.put("expectedNonRecurringIncome", "50000.0");
        requestData.put("bonusAmount", "invalid_bonus");
        ...
        assertNotNull(result.getBody());
        assertFalse((Boolean) result.getBody().get("success"));
        assertEquals("Invalid bonus amount format", result.getBody().get("message"));
    }

    @Test
    void testCalculateTaxEstimation_InvalidNumberOfBonus() {
        // Given
        when(mockAuth.getName()).thenReturn("123");
        ...
        requestData.put("expectedNonRecurringIncome", "50000.0");
        requestData.put("bonusAmount", "10000.0");
        requestData.put("numberOfBonus", "invalid_number");
        ...
        assertNotNull(result.getBody());
        assertFalse((Boolean) result.getBody().get("success"));
        assertEquals("Invalid number of bonus format", result.getBody().get("message"));
    }

    @Test
    void testCalculateTaxEstimation_InvalidInvestmentAmount() {
        // Given
        when(mockAuth.getName()).thenReturn("123");
        ...
        requestData.put("bonusAmount", "10000.0");
        requestData.put("numberOfBonus", "2");
        requestData.put("makeMoreInvestment", "invalid_investment");
        ...
        assertNotNull(result.getBody());
        assertFalse((Boolean) result.getBody().get("success"));
        assertEquals("Invalid investment amount format", result.getBody().get("message"));
    }

    @Test
    void testCalculateTaxEstimation_NullValues() {
        // Given
        when(mockAuth.getName()).thenReturn("123");

        Map<String, Object> requestData = new HashMap<>();
        requestData.put("expectedNonRecurringIncome", null);
        requestData.put("bonusAmount", null);
        requestData.put("numberOfBonus", null);
        requestData.put("makeMoreInvestment", null);

        when(jdbcTemplate.queryForObject(anyString(), eq(Double.class), eq(123), any(java.sql.Date.class)))
                .thenReturn(0.0);

        when(jdbcTemplate.queryForList(anyString(), eq(123), any(java.sql.Date.class)))
                .thenReturn(new ArrayList<>());

        List<Map<String, Object>> userTaxInfo = new ArrayList<>();
        Map<String, Object> taxInfo = new HashMap<>();
        taxInfo.put("is_ff", false);
        taxInfo.put("is_female", false);
        taxInfo.put("is_disabled", false);
        userTaxInfo.add(taxInfo);

        when(jdbcTemplate.queryForList(anyString(), eq(123))).thenReturn(userTaxInfo);
        when(jdbcTemplate.queryForObject(anyString(), eq(String.class), eq(123))).thenReturn("Rest");
        when(jdbcTemplate.queryForObject(anyString(), eq(Double.class), eq("Rest"))).thenReturn(0.0);
    ...
    @Test
    void testCalculateTaxEstimation_DailyRecurringIncome() {
        // Given
        when(mockAuth.getName()).thenReturn("123");

        Map<String, Object> requestData = new HashMap<>();
        requestData.put("expectedNonRecurringIncome", "0");
        requestData.put("bonusAmount", "0");
        requestData.put("numberOfBonus", "0");
        requestData.put("makeMoreInvestment", "0");
        ...
        dailyIncome.put("amount", 500.0);
        dailyIncome.put("recurrence", "daily");
        recurringIncomes.add(dailyIncome);
    ...
    @Test
    void testCalculateTaxEstimation_AnnualRecurringIncome() {
        // Given
        when(mockAuth.getName()).thenReturn("123");
        ...
        List<Map<String, Object>> recurringIncomes = new ArrayList<>();
        Map<String, Object> annualIncome = new HashMap<>();
        annualIncome.put("amount", 120000.0);
        annualIncome.put("recurrence", "annual");
        recurringIncomes.add(annualIncome);
    ...
    @Test
    void testCalculateTaxEstimation_FreedomFighterCategory() {
        // Given
        when(mockAuth.getName()).thenReturn("123");

        Map<String, Object> requestData = new HashMap<>();
        requestData.put("expectedNonRecurringIncome", "100000");
    ...
    @Test
    void testCalculateTaxEstimation_DisabledCategory() {
        // Given
        when(mockAuth.getName()).thenReturn("123");
    ...
    @Test
    void testCalculateTaxEstimation_TaxAreaNotFound() {
        // Given
        when(mockAuth.getName()).thenReturn("123");
        ...
        // Tax area query throws exception
        when(jdbcTemplate.queryForObject(anyString(), eq(String.class), eq(123)))
                .thenThrow(new EmptyResultDataAccessException(1));

        when(jdbcTemplate.queryForObject(anyString(), eq(Double.class), eq("Rest")))
                .thenReturn(0.0);
    ...
    @Test
    void testCalculateTaxEstimation_MinTaxNotFound() {
        // Given
        when(mockAuth.getName()).thenReturn("123");
        ...
        when(jdbcTemplate.queryForList(anyString(), eq(123))).thenReturn(userTaxInfo);
        when(jdbcTemplate.queryForObject(anyString(), eq(String.class), eq(123))).thenReturn("TestArea");

        // Min tax query throws exception
        when(jdbcTemplate.queryForObject(anyString(), eq(Double.class), eq("TestArea")))
                .thenThrow(new EmptyResultDataAccessException(1));
    ...
    @Test
    void testCalculateTaxEstimation_InvalidUserIdFormat() {
        // Given
        when(mockAuth.getName()).thenReturn("invalid_user_id");
    ...
    @Test
    void testCalculateTaxEstimation_NullAuthentication() {
        // Given
    ...
    @Test
    void testCalculateTaxEstimation_EmptyRequestData() {
        // Given
        when(mockAuth.getName()).thenReturn("123");

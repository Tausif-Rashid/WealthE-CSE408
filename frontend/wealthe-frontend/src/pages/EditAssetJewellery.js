...
import { getJewelleryItem, editJewellery } from '../utils/api';
import MessageDialog from '../components/MessageDialog';
import './AddExpense.css';

const EditAssetJewellery = () => {
  ...
  useEffect(() => {
    ...
    const fetchAsset = async () => {
      try {
        const jewellery = await getJewelleryItem(assetId);
        const asset = jewellery.data;
        console.log(asset);
        setFormData({
          title: asset.title || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await editJewellery(assetId, jewelleryData);
      showDialog('success', 'Success', 'Jewellery updated successfully!');
      
    } catch (err) {
      console.error('Error updating jewellery:', err);
      showDialog('error', 'Error', 'Failed to update jewellery. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="add-expense-container">
      <div className="add-expense-header">
        <h1>💍 Edit Jewellery</h1>
        <p>Update the details of your jewellery</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="add-expense-form">
          ...
          <div className="form-actions">
            ...
            <button type="submit" disabled={loading} className="add-add-btn">
              {loading ? 'Updating...' : 'Update Jewellery'}
            </button>
          </div>
        </form>
      </div>
...
export default EditAssetJewellery;
